...
export async function transferFlow(name?: string): Promise<number> {
  ...
  const recursive = await ui.confirm({ message: '📁 Рекурсивно (папка)?', default: false });

  const password = await resolvePassword(server);
  ui.printSection('📂', `scp ${direction} → ${targetSummary(server)}`);
  try {
    const code = await transfer(server, { direction, localPath, remotePath, recursive }, password);
    if (code === 0) ui.printOk('Готово.');
    else ui.printError(`scp завершился с кодом ${code}.`);
    return code;
  } catch (e) {
    ui.printError((e as Error).message);
    return 1;
