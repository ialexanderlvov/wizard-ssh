const ru = {
  ...
  labelTransferTool: (v: string) => `Перенос: инструмент по умолчанию: ${v}`,
  transferToolPrompt: 'Инструмент переноса по умолчанию',
  labelTransferRecursive: (v: string) => `Перенос (scp): рекурсивно: ${v}`,
  transferRecursivePrompt: '📁 scp: рекурсивно (папки) по умолчанию?',
  labelTransferCompress: (v: string) => `Перенос (rsync): сжатие -z: ${v}`,
  transferCompressPrompt: '🗜 rsync: сжимать в пути (-z) по умолчанию?',
  labelTransferDelete: (v: string) => `Перенос (rsync): --delete: ${v}`,
  transferDeletePrompt: '🧹 rsync: удалять лишнее на приёмнике (--delete) по умолчанию?',
