const ru = {
  ...
  exportOptForce: 'перезаписать файл, если он уже существует',
  ...
  completionInstallDesc: 'установить автодополнение (шелл определяется автоматически)',
  completionUninstallDesc: 'удалить установленное автодополнение',
  completionDetectFail: (shells: string) => `Не удалось определить шелл. Укажите явно: ${shells}.`,
  completionInstalledTitle: (shell: string) => `Автодополнение для ${shell} установлено`,
  completionWroteFile: (file: string) => `Файл: ${file}`,
  completionRcUpdated: (rc: string) => `Обновлён ${rc}`,
  completionOmzDetected: 'Обнаружен oh-my-zsh — файл добавлен в его $fpath.',
  completionReloadHint: (cmd: string) => `Перезапустите шелл или выполните: ${cmd}`,
  completionRemovedTitle: (shell: string) => `Автодополнение для ${shell} удалено`,
  completionRemovedFile: (file: string) => `Удалён ${file}`,
  completionNothingRemoved: 'Установленное автодополнение не найдено.',
